...
/**
 *
 * Description: A customized leaflet control to get the users location. Written in leaflets latest stable version, 1.7, released on November 17 and ES6 > .
 *
 *
 * Summary: TBT
 *
...
export function addGeoLocation(map, options){
    ...
    const findMeIcon = require('./assets/person-24px.svg');
    const geoLocation = L.Control.extend({
        options: {
            position: 'topleft',
            map: undefined,
            icon: findMeIcon,
            geoLocationClosed: true,
        },
