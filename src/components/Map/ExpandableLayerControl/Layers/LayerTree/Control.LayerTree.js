    ...
    L.Control.Layers.Tree = L.Control.Layers.extend({
        ...
        _initClassesNames() {
            this.cls = {
                ...
                checkbox: 'leaflet-layer-checkbox-container',
                ...
                nodeLine: 'leaflet-layertree-nodeline',
                nodeLineVertical: 'leaflet-layertree-nodeLine-vertical',
                nodeLineHorizontal: 'leaflet-layertree-nodeline-horizontal',
        ...
        expand() {
            const  ret = L.Control.Layers.prototype.expand.call(this);
        ...
        onAdd(map) {
            ...
            const ret = L.Control.Layers.prototype.onAdd.call(this, map);
        ...
        _iterateTreeLayout(tree, container, overlay, noShow) {
            if (!tree) return;
            function creator(type, cls, append, innerHTML) {
                const  obj = L.DomUtil.create(type, cls, append);
            ...
            const  sel = creator('span', this.cls.checkbox);
            ...
            const  {hide} = this.cls; // To toggle state
            ...
            if (tree.children) {
                ...
                tree.children.forEach(function(child) {
                    ...
                    const  nodeLine = creator('div', this.cls.nodeLine, node);
                    const  nodeLineVertical = creator('div', this.cls.nodeLineVertical, nodeLine);
                    const  nodeLineHorizontal = creator('div',  this.cls.nodeLineHorizontal, nodeLine);
            ...
            const  label = creator(tree.layer ? 'label' : 'span', this.cls.label, entry);
            if (tree.layer) {
                // now create the element like in _addItem
                const checked = this._map.hasLayer(tree.layer);
                let input;
                const radioGroup = overlay ? tree.radioGroup : `leaflet-base-layers_${  L.Util.stamp(this)}`;
                if (radioGroup) {
            ...
            const name = creator('span', this.cls.name, label, tree.label);
            L.DomUtil.addClass(closed, hide);
            if (noShow) {
